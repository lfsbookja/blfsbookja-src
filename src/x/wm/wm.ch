...
@x
    ...
    range of integrated utilities and applications.
@y
...
@y
    ...
    よく知られているものとして <application>fvwm2</application>, <application>Window
    Maker</application>, <application>AfterStep</application>, <application>Enlightenment</application>, <application>Sawfish</application>, <application>Blackbox</application> があります。
@z
...
@x
    ...
    <application>GNOME</application> have their own sections.
@y
    本章ではウィンドウマネージャーのいくつかと、一つの軽量デスクトップ環境について、そのインストール手順を示します。
    本章以降の別の章にて <application>KDE</application> と <application>GNOME</application> を説明しています。
@z
