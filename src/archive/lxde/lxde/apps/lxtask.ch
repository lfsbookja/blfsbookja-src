%
% This is part of BLFSbookja package.
%
% This is a CTIE change file for the original XML source of the BLFSbook.
%
% $Author$
% $Rev$
% $Date::                           $
%
@x
<?xml version="1.0" encoding="ISO-8859-1"?>
@y
<?xml version="1.0" encoding="UTF-8"?>
@z

@x
  <!ENTITY lxtask-time          "less than 0.1 SBU">
@y
  <!ENTITY lxtask-time          "&LessThan1;0.1 SBU&LessThan2;">
@z

@x
    <title>Introduction to LXTask</title>
@y
    <title>&IntroductionTo1;LXTask&IntroductionTo2;</title>
@z

@x
      The <application>LXTask</application> package contains a
      lightweight and desktop-independent task manager.
@y
      <application>LXTask</application> パッケージは軽量でデスクトップに依存しないタスクマネージャーを提供します。
@z

@x
    <bridgehead renderas="sect3">Package Information</bridgehead>
@y
    <bridgehead renderas="sect3">&PackageInformation;</bridgehead>
@z

@x
          Download (HTTP): <ulink url="&lxtask-download-http;"/>
@y
          &Download; (HTTP): <ulink url="&lxtask-download-http;"/>
@z

@x
          Download (FTP): <ulink url="&lxtask-download-ftp;"/>
@y
          &Download; (FTP): <ulink url="&lxtask-download-ftp;"/>
@z

@x
          Download MD5 sum: &lxtask-md5sum;
@y
          &Download; MD5 sum: &lxtask-md5sum;
@z

@x
          Download size: &lxtask-size;
@y
          &DownloadSize;: &lxtask-size;
@z

@x
          Estimated disk space required: &lxtask-buildsize;
@y
          &Estimateddiskspacerequired;: &lxtask-buildsize;
@z

@x
          Estimated build time: &lxtask-time;
@y
          &Estimatedbuildtime;: &lxtask-time;
@z

@x
    <bridgehead renderas="sect3">LXTask Dependencies</bridgehead>
@y
    <bridgehead renderas="sect3">&Dependencies1;LXTask&Dependencies2;</bridgehead>
@z

@x
    <bridgehead renderas="sect4">Required</bridgehead>
    <para role="required">
      <xref linkend="gtk2"/>
    </para>
@y
    <bridgehead renderas="sect4">&Required;</bridgehead>
    <para role="required">
      <xref linkend="gtk2"/>
    </para>
@z

@x
      User Notes: <ulink url="&blfs-wiki;/lxtask"/>
@y
      &UserNotes;: <ulink url="&blfs-wiki;/lxtask"/>
@z

@x
    <title>Installation of LXTask</title>
@y
    <title>&InstallationOf1;LXTask&InstallationOf2;</title>
@z

@x
      Install <application>LXTask</application> by running the following
      commands:
@y
      以下のコマンドを実行して <application>LXTask</application> をビルドします。
@z

@x
      This package does not come with a test suite.
@y
      &notTestSuite;
@z

@x
      Now, as the <systemitem class="username">root</systemitem> user:
@y
      <systemitem class="username">root</systemitem> ユーザーになって以下を実行します。
@z

@x
    <title>Contents</title>
@y
    <title>&Contents;</title>
@z

@x
      <segtitle>Installed Program</segtitle>
      <segtitle>Installed Libraries</segtitle>
      <segtitle>Installed Directories</segtitle>
@y
      <segtitle>&InstalledPrograms;</segtitle>
      <segtitle>&InstalledLibraries;</segtitle>
      <segtitle>&InstalledDirectories;</segtitle>
@z

@x
        <seg>
          lxtask
        </seg>
        <seg>
          None
        </seg>
        <seg>
          None
        </seg>
@y
        <seg>
          lxtask
        </seg>
        <seg>
          &None;
        </seg>
        <seg>
          &None;
        </seg>
@z

@x
      <bridgehead renderas="sect3">Short Descriptions</bridgehead>
@y
      <bridgehead renderas="sect3">&ShortDescriptions;</bridgehead>
@z

@x lxtask
            is a lightweight and desktop-independent task manager
@y
            軽量でデスクトップに依存しないタスクマネージャー。
@z